...
constexpr const char* readShaderFile(const char* filePath) {
    ...
    std::string content((std::istreambuf_iterator<char>(fl)), std::istreambuf_iterator<char>());

    // Dynamically allocate memory for the shader content
    char* shaderContent = new char[content.size() + 1];
    memcpy(shaderContent, content.c_str(), content.size() + 1);

    return shaderContent;
}

// Helper macro to read shader file at compile time
#define READ_SHADER_FILE(filePath) []() constexpr { \
    constexpr const char* shaderContent = readShaderFile(filePath); \
    return shaderContent ? shaderContent : ""; \
}()

// Usage example
constexpr const char* shaderCode = READ_SHADER_FILE();
